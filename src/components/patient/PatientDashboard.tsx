...
export const PatientDashboard: React.FC<PatientDashboardProps> = ({
  ...
  return (
    <div className="space-y-10 pb-16 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-3xl glass-panel p-8 sm:p-12 shadow-2xl transition-all">
        {/* Glow FX Backdrop */}
        <div className="absolute top-0 right-0 -mt-12 -mr-12 w-96 h-96 bg-rose-500/15 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute bottom-0 left-1/3 -mb-12 w-80 h-80 bg-sky-500/15 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute inset-0 medical-grid-bg opacity-75 pointer-events-none" />

        <div className="relative z-10 space-y-6 max-w-4xl">
          {/* Top Pill */}
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-800/80 text-rose-700 dark:text-rose-300 text-xs font-bold backdrop-blur-md shadow-sm">
          ...
          <p className="text-slate-600 dark:text-slate-300 text-sm sm:text-base leading-relaxed max-w-2xl font-normal">
            {t('welcomeBack')}{' '}
            <strong className="text-slate-900 dark:text-white font-bold">{currentUser?.displayName || 'User'}</strong>.{' '}
            {language === 'mr'
          ...
          <div className="flex flex-wrap items-center gap-4 pt-2">
            <button
              ...
              className="flex items-center gap-3 px-6 py-4 bg-gradient-to-r from-rose-600 via-red-600 to-rose-700 hover:from-rose-500 hover:to-red-500 text-white font-black text-sm rounded-2xl shadow-2xl shadow-rose-600/40 hover:scale-105 transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="flex items-center gap-3 px-6 py-4 bg-white dark:bg-slate-800/90 hover:bg-slate-100 dark:hover:bg-slate-700/90 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white font-bold text-sm rounded-2xl shadow-lg hover:scale-105 transition-all cursor-pointer"
            >
            ...
            <a
              ...
              className="flex items-center gap-2 px-5 py-4 bg-rose-50 dark:bg-rose-950/60 hover:bg-rose-100 dark:hover:bg-rose-900/60 border border-rose-200 dark:border-rose-800/80 text-rose-700 dark:text-rose-300 font-bold text-xs rounded-2xl transition-all shadow-sm"
            >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-card rounded-3xl p-5 shadow-xl flex items-center gap-4 hover:border-sky-500/40 transition-all">
          <div className="w-12 h-12 rounded-2xl bg-sky-50 dark:bg-sky-500/10 border border-sky-200 dark:border-sky-500/30 flex items-center justify-center text-sky-600 dark:text-sky-400 shrink-0 shadow-inner">
        ...
        <div className="glass-card rounded-3xl p-5 shadow-xl flex items-center gap-4 hover:border-indigo-500/40 transition-all">
          <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shrink-0 shadow-inner">
        ...
        <div className="glass-card rounded-3xl p-5 shadow-xl flex items-center gap-4 hover:border-emerald-500/40 transition-all">
          <div className="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shrink-0 shadow-inner">
        ...
        <div className="glass-card rounded-3xl p-5 shadow-xl flex items-center gap-4 hover:border-amber-500/40 transition-all">
          <div className="w-12 h-12 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 flex items-center justify-center text-amber-600 dark:text-amber-400 shrink-0 shadow-inner">
        ...
        <div className="flex flex-wrap gap-2">
          {(language === 'mr' ? [
            ...
            <button
              key={idx}
              onClick={() => onNavigate('ai_chat')}
              className="px-3.5 py-2 rounded-2xl bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-xs text-slate-700 dark:text-slate-200 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all font-medium text-left cursor-pointer"
            >
              {prompt}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card rounded-3xl p-6 shadow-xl space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Calendar className="w-5 h-5 text-rose-500" />
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">{t('myActiveAppointments')}</h2>
            </div>
            <button
          ...
          {activeUpcomingAppointment ? (
            <div className="bg-slate-50 dark:bg-slate-800/80 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-slate-700 flex items-center justify-center text-indigo-600 dark:text-rose-400 font-bold text-lg border border-indigo-200 dark:border-slate-600 shadow-inner">
                    <Stethoscope className="w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="font-bold text-slate-900 dark:text-white text-base">{activeUpcomingAppointment.doctorName}</h3>
                    <p className="text-slate-500 dark:text-slate-400 text-xs">{activeUpcomingAppointment.doctorSpecialization}</p>
                    <p className="text-slate-500 dark:text-slate-400 text-[11px] mt-0.5">{activeUpcomingAppointment.hospitalName}</p>
                  </div>
                </div>

                <div className="flex flex-col items-start sm:items-end">
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-semibold border border-emerald-500/30">
              ...
              <div className="pt-3 border-t border-slate-200 dark:border-slate-700/60 flex items-center justify-between">
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  ...
                  <strong className="text-slate-800 dark:text-white capitalize">{activeUpcomingAppointment.type.replace('_', ' ')}</strong>
                </div>
                ...
                {activeUpcomingAppointment.type === 'video' && activeUpcomingAppointment.meetingId && (
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-emerald-600/30 transition-all cursor-pointer animate-pulse"
                  >
            ...
            <div className="bg-slate-50 dark:bg-slate-800/40 rounded-2xl p-6 text-center border border-dashed border-slate-300 dark:border-slate-700 space-y-3">
              <p className="text-slate-500 dark:text-slate-400 text-xs">{t('noActiveAppointments')}</p>
              <button
                onClick={() => onNavigate('doctors')}
                className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold rounded-xl transition-all shadow-md cursor-pointer"
              >
                {t('bookSpecialist')}
              </button>
            </div>
        ...
        <div className="glass-card rounded-3xl p-6 shadow-xl space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">{t('medicalProfileSummary')}</h2>
            </div>
            <button
          ...
          <div className="bg-slate-50 dark:bg-slate-800/80 rounded-2xl p-4 border border-slate-200 dark:border-slate-700 space-y-3 text-xs">
            <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-700">
              ...
              <span className="px-2.5 py-1 bg-red-500/20 text-red-600 dark:text-red-400 border border-red-500/40 rounded-lg font-extrabold text-sm">
              ...
              <div className="flex flex-wrap gap-1.5">
                {medicalProfile.allergies.length > 0 ? (
                  medicalProfile.allergies.map((allergy, i) => (
                    <span key={i} className="px-2 py-0.5 bg-amber-500/20 text-amber-600 dark:text-amber-300 border border-amber-500/30 rounded-md text-[11px]">
                      ⚠️ {allergy}
                    </span>
                  ))
                ) : (
                  <span className="text-slate-400 italic">{language === 'mr' ? 'काहीही नाही' : 'None listed'}</span>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-emerald-600 dark:text-emerald-400 font-mono text-[11px] font-semibold">+91 98765 43210</p>
            </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t('verifiedGuides')}</h2>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {firstAidArticles.slice(0, 4).map(article => (
            <div
              ...
              className="glass-card glass-card-hover rounded-2xl p-5 cursor-pointer flex flex-col justify-between space-y-3 group"
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="w-10 h-10 rounded-xl bg-red-50 dark:bg-red-950/60 border border-red-200 dark:border-red-500/30 flex items-center justify-center text-red-600 dark:text-red-400 shadow-inner">
                    {article.articleId === 'fa_cpr' && <HeartPulse className="w-5 h-5" />}
                    {article.articleId === 'fa_snake_bite' && <ShieldAlert className="w-5 h-5 text-amber-500" />}
                    {article.articleId === 'fa_heart_attack' && <Activity className="w-5 h-5 text-rose-500" />}
                    {article.articleId === 'fa_burns' && <Flame className="w-5 h-5 text-orange-500" />}
                    {article.articleId === 'fa_asthma' && <Wind className="w-5 h-5 text-sky-500" />}
                  </div>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${
                    article.emergencyLevel === 'Critical' ? 'bg-red-500/15 text-red-600 dark:text-red-400 border border-red-500/40' : 'bg-amber-500/15 text-amber-600 dark:text-amber-300 border border-amber-500/40'
                  }`}>
                    {article.emergencyLevel}
                  </span>
                </div>

                <h3 className="font-bold text-slate-900 dark:text-white text-sm group-hover:text-rose-500 transition-colors line-clamp-1">
                  {article.title}
                </h3>
