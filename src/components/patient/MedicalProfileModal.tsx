      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-red-50 dark:bg-red-950/60 border border-red-200 dark:border-red-500/30 flex items-center justify-center text-red-600 dark:text-red-400 shadow-inner">
              <HeartPulse className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t('patientMedicalProfile')}</h2>
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-slate-700 dark:text-slate-300 text-sm">
          ...
          <div className="bg-indigo-50/60 dark:bg-indigo-950/30 p-4 rounded-2xl border border-indigo-200 dark:border-indigo-500/30 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`py-3 px-4 rounded-xl text-xs font-extrabold border transition-all flex items-center justify-center gap-2 cursor-pointer ${
                  selectedLanguage === 'en'
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs rounded-xl transition-all flex items-center gap-1 cursor-pointer"
              >
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {allergies.map((allergy, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30 text-xs"
                >
                  ⚠️ {allergy}
                  <button type="button" onClick={() => handleRemoveAllergy(i)}>
                    <Trash2 className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 hover:text-rose-600" />
                  </button>
                </span>
              ))}
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-slate-700 font-bold text-xs rounded-xl transition-all flex items-center gap-1 cursor-pointer"
              >
                <Plus className="w-4 h-4" /> {t('addBtn')}
              </button>
            </div>
            <div className="flex flex-wrap gap-2 pt-1">
              {conditions.map((cond, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-sky-500/20 text-sky-700 dark:text-sky-300 border border-sky-500/30 text-xs"
                >
                  🩺 {cond}
                  <button type="button" onClick={() => handleRemoveCondition(i)}>
                    <Trash2 className="w-3.5 h-3.5 text-sky-600 dark:text-sky-400 hover:text-rose-600" />
                  </button>
                </span>
              ))}
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {medications.map((med, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-500/30 text-xs"
                >
                  💊 {med}
                  <button type="button" onClick={() => handleRemoveMedication(i)}>
                    <Trash2 className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400 hover:text-rose-600" />
                  </button>
                </span>
              ))}
            </div>
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleSave}
              className="px-5 py-2.5 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-red-600/30 transition-all flex items-center gap-2 cursor-pointer"
            >
