...
export const AIAssistant: React.FC<AIAssistantProps> = ({ onFindHospitals, onReadFirstAid }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12 animate-fade-in">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-slate-900 dark:text-white">{t('aiTitle')}</h1>
              <span className="px-2 py-0.5 rounded-md bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 text-[10px] font-bold font-mono">
                {selectedModel} Powered
              </span>
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400">{t('aiSubtitle')}</p>
          </div>
        </div>

        {/* Model Switcher & Clear Controls */}
        <div className="flex items-center gap-2">
          <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 text-xs shadow-sm">
            <button
              onClick={() => setSelectedModel('Gemini')}
              className={`px-3 py-1 rounded-lg font-bold transition-all ${
                selectedModel === 'Gemini' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
              Gemini 1.5
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg font-bold transition-all ${
                selectedModel === 'OpenAI' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
              OpenAI gpt-4o
            </button>
          ...
          <button
            ...
            className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-rose-100 dark:hover:bg-rose-950/60 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 transition-colors shadow-sm cursor-pointer"
            title={t('clearChat')}
          >
      ...
      {/* Chat Area Container */}
      <div className="glass-panel rounded-3xl h-[560px] flex flex-col shadow-2xl overflow-hidden">
        {/* Messages Body */}
        <div className="flex-1 p-6 overflow-y-auto space-y-6">
          {/* Medical Safety Disclaimer */}
          <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-500/30 rounded-2xl p-4 flex items-start gap-3 text-xs text-amber-800 dark:text-amber-200 shadow-sm">
                ...
                <div className={`max-w-2xl space-y-3 ${isAI ? 'w-full' : ''}`}>
                  <div
                    className={`p-5 rounded-3xl text-sm leading-relaxed ${
                      isAI
                        ? 'glass-card text-slate-800 dark:text-slate-100 shadow-xl'
                        : 'bg-gradient-to-r from-rose-600 to-red-600 text-white shadow-lg shadow-rose-600/30 rounded-br-none'
                    }`}
                  >
                    {/* Severity Badge */}
                        ...
                        <span
                          className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold uppercase ${
                            msg.severity === 'CRITICAL'
                              ? 'bg-red-500/20 text-red-600 dark:text-red-300 border border-red-500/50 animate-pulse'
                    ...
                    {msg.avoid && msg.avoid.length > 0 && (
                      <div className="mt-3 pt-2 border-t border-slate-200 dark:border-slate-700/80 space-y-1">
                        ...
                        <button
                          ...
                          className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all shadow-md flex items-center gap-1.5 cursor-pointer ${
                            action.action === 'call_emergency'
                              ...
                              : 'glass-card hover:bg-slate-100 dark:hover:bg-slate-700 text-emerald-600 dark:text-emerald-400'
                          }`}
                        >
          ...
          {isProcessing && (
            <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400 text-xs italic">
              <div className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-emerald-500 animate-spin">
        ...
        {/* Input Bar */}
        <div className="p-4 bg-slate-50/90 dark:bg-slate-900/90 border-t border-slate-200 dark:border-slate-800 space-y-3">
          ...
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-none pb-1 text-xs">
            <span className="text-slate-500 dark:text-slate-400 text-[11px] font-bold shrink-0">
            ...
            <button
              ...
              className="px-3 py-1 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg whitespace-nowrap transition-colors border border-slate-200 dark:border-slate-700 shadow-sm cursor-pointer"
            >
            ...
            <button
              ...
              className="px-3 py-1 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-amber-700 dark:text-amber-300 rounded-lg whitespace-nowrap transition-colors border border-slate-200 dark:border-slate-700 shadow-sm cursor-pointer"
            >
            ...
            <button
              ...
              className="px-3 py-1 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-rose-700 dark:text-rose-300 rounded-lg whitespace-nowrap transition-colors border border-slate-200 dark:border-slate-700 shadow-sm cursor-pointer"
            >
          ...
          <div className="flex items-center gap-2">
            <label className="p-2.5 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white rounded-2xl cursor-pointer border border-slate-200 dark:border-slate-700 transition-colors shadow-sm">
            ...
            <input
              ...
              className="flex-1 glass-input rounded-2xl px-4 py-3 text-xs"
            />

            <button
              onClick={() => handleSend()}
              disabled={isProcessing}
              className="px-5 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-xs rounded-2xl shadow-lg shadow-emerald-600/30 transition-all flex items-center gap-1.5 disabled:opacity-50 cursor-pointer"
            >
